...
public class Promocao implements Serializable {
	...
	public void registra(Lance lance) {
		...
		if(lance.getValor() > valorMaximo){
			throw new RuntimeException("Lance excedeu valor maximo da promocao");
		}
		
		if(clienteExcedeuONumeroMaximoDeLances(lance.getCliente()) ||
		   clienteDandoDoisLancesEmSequencia(lance.getCliente())){
			return;
		}
	...
	private boolean clienteExcedeuONumeroMaximoDeLances(Cliente cliente) {
		int total = 0;
	...
	private Cliente clienteRegistrouUltimoLance(){
		return lances.get(lances.size()-1).getCliente();
	}
	
	private boolean clienteDandoDoisLancesEmSequencia(Cliente cliente){
		return !lances.isEmpty() && 
			   clienteRegistrouUltimoLance().equals(cliente);
	}
